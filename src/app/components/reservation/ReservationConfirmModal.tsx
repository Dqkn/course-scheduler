...
import type { Room, ReservationFilterState, Reservation } from '../../types/reservationTypes';
import {
  formatDateTr,
  formatDateEn,
  getRoomTypeLabel,
...
export function ReservationConfirmModal({
  ...
  function handleConfirm() {
    // Double-check availability for each slot individually
    for (const slot of filters.timeSlots) {
      if (!isRoomAvailable(room.roomCode, filters.date, [slot])) {
        setState('conflict');
        return;
      }
    }
    ...
    setState('loading');
    // Simulate network delay, then save each time slot as a separate reservation
    setTimeout(() => {
