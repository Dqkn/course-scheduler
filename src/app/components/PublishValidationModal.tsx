import {
  ...
  Shield, BookOpen, Building2, User, Users, Timer, BarChart3, Pin,
} from 'lucide-react';
...
export function PublishValidationModal({ onClose, onConfirm }: PublishValidationModalProps) {
  const {
    darkMode, scheduledCourses, scheduleStats, pinnedCourseIds,
  } = useApp();
  const { t } = useLocale();
  ...
  const checks = useMemo<ValidationCheck[]>(() => {
    ...
    const noInstructor = courses.filter(c => !c.lecturer || c.lecturer.trim() === '' || c.lecturer.toLowerCase() === 'anonim').length;
    result.push({
      id: 'instructor',
    ...
    const overCapacity = courses.filter(c => {
    ...
    // Group courses by lecturer+day, sort by start time, count max consecutive
    const lecturerDayMap: Record<string, { start: number; end: number }[]> = {};
    ...
    for (const [key, slots] of Object.entries(lecturerDayMap)) {
      ...
      for (let i = 1; i < slots.length; i++) {
        if (slots[i].start <= blockEnd) {
          ...
          totalHours = Math.max(totalHours, blockEnd - blockStart);
      ...
      totalHours = Math.max(totalHours, blockEnd - blockStart);
      if (totalHours >= 6) {
        const lecturerName = key.split('__')[0];
        overConsecutive.add(lecturerName);
      }
    }
    result.push({
      id: 'consecutive',
    ...
    // 8. Pinned Courses (info)
    const pinnedCount = pinnedCourseIds.length;
    result.push({
      id: 'pinned',
      title: t.publishValidation.pinnedTitle,
      message: pinnedCount === 0
        ? t.publishValidation.pinnedNone
        : t.publishValidation.pinnedInfo.replace('{n}', String(pinnedCount)),
      status: pinnedCount === 0 ? 'passed' : 'info',
      icon: <Pin className="w-4.5 h-4.5" />,
      critical: false,
    });

    return result;
  }, [scheduledCourses, scheduleStats, pinnedCourseIds, t]);
