...
import type { AlgorithmInput, ScheduleStats, SchedulerResult, TermType } from '../types/schedulerTypes';
...
function canPlace(task: Task, day: number, slot: number, assigns: Assignment[]): boolean {
  ...
  for (const a of assigns) {
    ...
    if (!overlap) continue;
    if (task.lecturer !== 'anonim' && task.lecturer.trim() !== '-' && task.lecturer === a.task.lecturer) return false;
...
function schedule(tasks: Task[]): Assignment[] {
  ...
  for (const task of sorted) {
    ...
    for (const d of shuffle([0, 1, 2, 3, 4])) {
      ...
      for (const s of shuffle(Array.from({ length: TIMESLOTS }, (_, i) => i))) {
        if (canPlace(task, d, s, assigns)) { assigns.push({ task, day: d, startSlot: s }); placed = true; break; }
      }
    }
    if (!placed) {
...
export function runDemoScheduler(inputs: AlgorithmInput[], term: TermType): SchedulerResult {
  const start = performance.now();
  const filtered = inputs.filter(c => term === 'fall' ? c.course_semester % 2 !== 0 : c.course_semester % 2 === 0);
  const tasks = expand(filtered);
  const assigns = schedule(tasks);
  let courses = toCourses(assigns);
